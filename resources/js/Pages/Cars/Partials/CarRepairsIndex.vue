        ...
        <div class="flex flex-wrap justify-between items-center p-2 mt-4">
            <div class="mx-auto mt-2">
                <div class="relative ">
                    ...
                    <input type="text" id="search" v-model="form.search" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-red-500 focus:border-red-500 block w-full pl-10 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-red-500 dark:focus:border-red-500" placeholder="Buscar">
                </div>
            </div>
            <div class="mx-auto mt-2">
            ...
            <div class="space-x-2 space-y-2 mx-auto">
                <Link 
        ...
        <div class="flex flex-wrap justify-between gap-4 my-12">
            <CardSection v-for="repair in props.repairs.data" class="text-base p-2">
                <Link  v-if="hasPermission('ver reparacion')"
                    ...
                    <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{repair.work_shop?.name ?? 'Taller Eliminado'}},{{repair.car_id}}</h5>
                    ...
                    <span class="font-semibold">Creado el:</span> {{ repair.created_date }}
                </Link>
                <div v-else
                    class="block max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700"
                >
                    <h5 class="mb-2 text-lg font-bold tracking-tight text-gray-900 dark:text-white">{{repair.work_shop?.name ?? 'Taller Eliminado'}},{{repair.car_id}}</h5>
                    <p class="font-normal text-gray-700 dark:text-gray-400">{{repair.status?.name}}</p>
                    <span class="font-semibold">Total:</span> ${{ repair.total }} <br>
                    <span class="font-semibold">Creado el:</span> {{ repair.created_date }}
                </div>
