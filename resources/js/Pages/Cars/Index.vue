...
watch(form,debounce(() => {
    router.get('/cars', {search:form.search, trashed:form.trashed}, { preserveState:true , replace:true });
},500));
                        ...
                        <slot>
                            <tr v-for="car in props.cars.data" :key="car.id" class="hover:bg-gray-200 dark:hover:bg-gray-500  bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                                <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white flex items-center">
                                    <svg fill="none" v-if="car.deleted_at != null" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" class="flex-shrink-0 ml-2 w-3 h-3 fill-gray-400">
                            ...
                            <tr v-if="props.cars.data.length === 0">
                                <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-gray-200" colspan="4">No Autos Encontrados.</td>
                            </tr>
                        </slot>
                    </TableComponent>
