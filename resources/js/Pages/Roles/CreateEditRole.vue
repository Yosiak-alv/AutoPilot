...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">{{ props.role == null ? 'Crear un Nuevo Rol': 'Editar Rol'}}</h2>
        </template>
        ...
        <div class="py-12">
            <CardSection class="max-w-7xl mt-6 space-y-6">
                <form @submit.prevent="(props.role == null ? store(): update())" class="grid grid-cols-2 gap-4 p-6">
                    <div>
                        <div class="mt-1">
                            <InputLabel for="name" value="Nombre de Rol" />
