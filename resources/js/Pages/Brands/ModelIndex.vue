    ...
    <div v-if="props.models.length != 0 ">
        <div class="max-w-7xl mx-auto text-center my-4 text-gray-900 dark:text-gray-100">
            ...
            <div class="flex justify-between items-center p-2 mt-4">
                <div>
                    <PrimaryButton @click="confirmModelCreation()" v-if="hasPermission('crear modelo')">
                        + Modelo
                    </PrimaryButton>
                </div>
            </div>
        ...
        <div class="flex flex-wrap justify-between gap-4 my-12">
            <CardSection v-for="model in props.models" :key="model.id" class="text-base p-2">
                <Link v-if="hasPermission('ver modelo')" 
                    method="get" as="button" :href="route('brands.showModel',model.id)" class="block max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
                    <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{model.name}}</h5>
                </Link>
                <div v-else class="block max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
                    <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{model.name}}</h5>
                </div>
            </CardSection>
        </div>
