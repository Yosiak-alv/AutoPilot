...
watch(form,debounce(() => {
    router.get('/workshops', {search:form.search , trashed:form.trashed}, { preserveState:true , replace:true });
},500));
            ...
            <CardSection class="max-w-7xl">
                <div class="flex flex-wrap justify-between items-center p-5 ">
                    ...
                    <div class="space-x-2 space-y-2">
                        <PrimaryButton class="ml-2" v-if="hasPermission('crear taller')"  @click="createWorkshops()" >Agregar Taller</PrimaryButton>

                        <a :href="route('workshops.excelIndexExport',{_query: {
                                                                search: form.search,
                                                                trashed: form.trashed
                                                            },})" 
                            v-if="hasPermission('exportar a excel') && props.work_shops.data.length != 0"    
                ...
                <div class="p-5">
                    <TableComponent class="rounded-lg bg-gray-100 dark:bg-gray-700">
                        ...
                        <slot>
                            <tr v-for="work_shop in props.work_shops.data" :key="work_shop.id" class="hover:bg-gray-200 dark:hover:bg-gray-500  bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                                <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white ">
                                    <div class="flex items-center">
                                        {{work_shop.name}}
                                        <svg fill="none" v-if="work_shop.deleted_at != null" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" class="flex-shrink-0 ml-2 w-3 h-3 fill-gray-400">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <Link v-if="hasPermission('ver taller')" :href="route('workshops.show',{id: work_shop.id})">
                            ...
                            <tr v-if="props.work_shops.data.length === 0">
                                <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-gray-200" colspan="4">No Talleres Encontrados.</td>
                            </tr>
                        </slot>
                    </TableComponent>
