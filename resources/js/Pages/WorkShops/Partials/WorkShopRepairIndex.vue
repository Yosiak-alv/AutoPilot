...
watch(search, debounce((value) => {
    router.get(route('workshops.show',{id:props.workShopId}), {search:value}, { preserveState:true , replace:true ,preserveScroll:true});
},500));
...
<template>
    <h2 class="text-3xl font-semibold text-center">Reparaciones</h2>
        ...
        <div class="flex flex-wrap justify-between gap-4 my-12">
            <CardSection v-for="repair in props.repairs.data" class="text-base p-2">
                <Link  v-if="hasPermission('ver reparacion')"
                    ...
                    <h5 class="mb-2 text-lg font-bold tracking-tight text-gray-900 dark:text-white">{{repair.car.model?.name}}, {{repair.car?.model.brand.name}},{{repair.work_shop_id}}</h5>
                    <p class="font-normal text-gray-700 dark:text-gray-400">{{repair.car?.plates}}</p>
                    <p class="font-normal text-gray-700 dark:text-gray-400">{{repair.car?.year}}</p>
                ...
                >
                    <h5 class="mb-2 text-lg font-bold tracking-tight text-gray-900 dark:text-white">{{repair.car.model?.name}}, {{repair.car?.model.brand.name}}</h5>
                    <p class="font-normal text-gray-700 dark:text-gray-400">{{repair.car?.plates}}</p>
                    <p class="font-normal text-gray-700 dark:text-gray-400">{{repair.car?.year}}</p>
                    <p class="font-normal text-gray-700 dark:text-gray-400">{{repair.status?.name}}</p>
                    <span class="font-semibold">Total:</span> ${{ repair.total }}
    ...
    <div v-else>
        <div class="max-w-7xl mx-auto text-center my-4 text-gray-900 dark:text-gray-100">
            <h2 class="text-3xl font-semibold">No hay reparaciones en este Taller.</h2>
        </div>
    </div>
